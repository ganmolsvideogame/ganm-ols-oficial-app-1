...
export async function POST(request: Request) {
  ...
  const buyerZipcode = normalizeZipcode(payload.zipcode);
  ...
  const { data: listing, error } = await supabase
    .from("listings")
    .select(
      "id, title, price_cents, seller_user_id, shipping_available, free_shipping, family, package_weight_grams, package_length_cm, package_width_cm, package_height_cm"
    )
    .eq("id", listingId)
    .maybeSingle();
  ...
  try {
    ...
    const quote = options[0] ?? (await calculateShipping({
      fromZipcode: sellerZipcode,
      toZipcode: buyerZipcode,
      weightGrams: dimensions.weightGrams,
      lengthCm: dimensions.lengthCm,
