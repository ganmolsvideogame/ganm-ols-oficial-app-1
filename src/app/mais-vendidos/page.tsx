...
export default async function Page() {
  ...
  // Ainda nao existe ranking real de vendas. Enquanto isso, mostramos destaques + mais recentes.
  const { data } = await supabase
    .from("listings_with_boost")
    .select(
      "id, title, price_cents, condition, family, platform, shipping_available, free_shipping, thumbnail_url, boost_priority"
    )
    .eq("status", "active")
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-zinc-900">Mais vendidos</h1>
        <p className="mt-2 text-sm text-zinc-600">
          Selecao de anuncios em alta e novidades da vitrine.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {listings.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-zinc-200 bg-zinc-50 p-6 text-sm text-zinc-500">
            Nenhum anuncio encontrado no momento.
          </div>
        ) : (
          listings.map((item) => (
            <ListingCard
              ...
              tag={item.boost_priority ? "Em alta" : undefined}
            />
          ))
        )}
      </div>
